  ...
  <div>
    ...
    <card class="mb-6 py-3 px-6">
      <transition name="fade">
        ...
        >
          <svg
            class="w-16 h-16 fill-current text-danger mb-4"
            viewBox="0 0 640 512"
      ...
      <div v-if="!currentlyInMaintenance">
        <div class="flex">
          <div class="w-1/4 py-4">
            <h4 class="font-normal text-80">{{ __('Mesaj') }}</h4>
          </div>
          <div class="w-3/4 py-4">
            <input
              class="form-control form-input w-1/2 "
              name="message"
              :placeholder="__('Message for visitors')"
              type="text"
              v-model="message"
            >
          </div>
        ...
        <!--                    <div class="w-1/4 py-4">-->
        <!--                        <h4 class="font-normal text-80">Retry Time-->
        <!--                            <span class="text-sm">(secs)</span>-->
        <!--                        </h4>-->
        <!--                    </div>-->
        <!--                    <div class="w-3/4 py-4">-->
        <!--                        <input class="form-control form-input form-input-bordered w-1/4" max=9999 min=0-->
        <!--                               name="retry"-->
        <!--                               placeholder="Set the Retry-After header" type="number" v-model="retry">-->
        <!--                    </div>-->
        <!--                </div>-->
        <!--                <div class="flex border-b border-40">-->
        <!--                    <div class="w-1/4 py-4">-->
        <!--                        <h4 class="font-normal text-80">İzin verilen ip adresleri</h4>-->
        <!--                    </div>-->
        <!--                    <div class="w-3/4 py-4">-->
        <!--                        <input class="form-control form-input form-input-bordered w-1/2" name="allow" placeholder="127.0.0.1, 192.168.0.1"-->
        <!--                               type="text"-->
        <!--                               v-model="allow">-->
        <!--                    </div>-->
        <!--                </div>-->
        ...
        <!--                            Mevcut IP adresi dahil et-->
        <!--                        </h4>-->
        <!--                    </div>-->
        <!--                    <div class="w-3/4 py-4">-->
        <!--                        <toggle-button class="mb-4" height="22" speed="400" v-model="include_current_ip" width="50" />-->
        <!--                        <p class="text-danger">NOTE</p>-->
        <!--                        <p class="text-80"> If you uncheck this and do not add your IP address above you will lose-->
        <!--                            access to Nova as well-->
        <!--                        </p>-->
        <!--                    </div>-->
        <!--                </div>-->
      </div>
...
export default {
  ...
  metaInfo() {
    return {
      title: this.__("Maintenance Mode"),
    };
  },
  methods: {
    enableMaintenance() {
      Nova.request()
        .post("/nova-vendor/Davidpiesse/nova-maintenance-mode/down", {
        ...
        .then((response) => {
          ...
          Nova.$emit("maintenance-mode", true);
        });
    },
    disableMaintenance() {
      Nova.request()
        .post("/nova-vendor/Davidpiesse/nova-maintenance-mode/up")
        .then((response) => {
          this.currentlyInMaintenance = response.data.currentlyInMaintenance;
          this.$toasted.show(response.data.message, { type: "success" });

          Nova.$emit("maintenance-mode", false);
        });
    },
