      ...
      >
        ...
        >
          <div class="flex items-center my-2">
            <h3 class="mr-2">{{ __(group) }}</h3>
            <!-- <checkbox
              :value="group"
              :checked="isCheckedGroup(group,permissions)"
              @input="toggleGroupOption(group)"
              class="pl-2"
            /> -->
          </div>
          <div
...
export default {
  ...
  methods: {
    ...
    isCheckedGroup (group, permissions) {
      let grouppedPermissions = this.groupPermissions(permissions)[group];

      console.log(grouppedPermissions,this.value);

      return _.every(this.value, grouppedPermissions);
    },
    isChecked (option) {
    ...
    groupPermissions (permissions) {
      let groups = _.groupBy(permissions, (perm) => {
        return perm.group;
      });

      return _.map(groups, (permissions, group) => {
        return {
          [group]: _.map(permissions, (perm) => {
            return perm.option;
          })
        }
      })[0] || {};
